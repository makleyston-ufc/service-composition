...
SERVICE_NAME = "social-accessibility-index"
...
DEFAULT_OUTPUT_TOPIC = "service/social/inference/social-accessibility-index"
DEFAULT_SERVICE_URN = "urn:ufcity:service:social-accessibility-index"
DEFAULT_PROPERTY_KIND = "urn:ufcity:propertykind:SocialAccessibilityIndex"
...
class SocialAccessibilityIndexService(Observer):
    def __init__(
        self,
        *,
        ...
        self.features = model_artifact.get("features")
        if not self.features:
            raise ValueError("Artifact de modelo invalido: esperado 'features'")

        self.index_weights = model_artifact.get("index_weights", {"baixa": 0.0, "media": 50.0, "alta": 100.0})
        self.static_context = model_artifact.get("cras_info", {})
    ...
    def _parse_mobility_efficiency_input(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "mobility_class": result.get("saref:hasValue"),
            "mobility_confidence": _safe_float(result.get("urn:ufcity:confidence")),
            "efficiency_index": _safe_float(result.get("urn:ufcity:efficiencyIndex"))
            if result.get("urn:ufcity:efficiencyIndex") is not None
            else _safe_float(payload.get("efficiency_index")),
            "traffic_status": inputs_summary.get("traffic_status"),
            "bus_status": inputs_summary.get("bus_status"),
            "mob_prob_baixa": _safe_float(class_probs.get("baixa")),
            "mob_prob_media": _safe_float(class_probs.get("media")),
            "mob_prob_alta": _safe_float(class_probs.get("alta")),
        }
    ...
    def _parse_regional_pressure_input(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "press_prob_baixa": _safe_float(class_probs.get("baixa")),
            "press_prob_media": _safe_float(class_probs.get("media")),
            "press_prob_alta": _safe_float(class_probs.get("alta")),
        }

    def _build_model_input(self) -> pd.DataFrame:
        mobility_topic = self.input_topics[0]
        pressure_topic = self.input_topics[1]
        mobility_features = self.latest_by_topic[mobility_topic]["features"]
        pressure_features = self.latest_by_topic[pressure_topic]["features"]

        timestamps = [
            self.latest_by_topic[mobility_topic]["timestamp"],
            self.latest_by_topic[pressure_topic]["timestamp"],
        ]
        timestamp = max(timestamps)
        parsed = pd.to_datetime(timestamp, errors="coerce")
        hour = int(parsed.hour) if not pd.isna(parsed) else None
        weekday = int(parsed.dayofweek) if not pd.isna(parsed) else None

        row = {**mobility_features, **pressure_features, "hour": hour, "weekday": weekday}
        for key, value in self.static_context.items():
            if key == "cras_dataset_file":
                continue
    ...
    def _compute_accessibility_index(self, proba: np.ndarray, classes: list[str]) -> float:
    ...
    def _build_output_message(
        ...
        *,
        ...
        accessibility_index: float | None,
        ...
        payload: dict[str, Any] = {
            "inputs_summary": {
                ...
                "bus_status": self.latest_by_topic[self.input_topics[0]]["features"].get("bus_status"),
        ...
        if accessibility_index is not None:
            payload["social_accessibility_index"] = round(accessibility_index, 3)
        ...
        if accessibility_index is not None:
            message["saref:hasResult"]["urn:ufcity:socialAccessibilityIndex"] = round(
                float(accessibility_index), 3
            )
    ...
    def update(self, topic: str, message: Any) -> None:
        ...
        if topic == self.input_topics[0]:
            parsed_features = self._parse_mobility_efficiency_input(message)
        else:
        ...
        if hasattr(self.pipeline, "predict_proba"):
            ...
            accessibility_index = self._compute_accessibility_index(proba=proba, classes=classes)
        ...
        print(
            f"[{SERVICE_NAME}] publicado acessibilidade={prediction} "
            f"index={None if accessibility_index is None else round(accessibility_index, 3)} "
...
def main() -> None:
    ...
    if len(input_topics) != 2:
        raise ValueError(
            "Este servico exige exatamente 2 topicos de entrada: mobility-efficiency-index e regional-mobility-pressure"
        )
    ...
    if not isinstance(model_artifact, dict):
        raise ValueError("Artifact de modelo invalido: esperado dict")

    publisher = MqttPublish({"broker_address": mqtt_host, "port": mqtt_port})
    observer = SocialAccessibilityIndexService(
