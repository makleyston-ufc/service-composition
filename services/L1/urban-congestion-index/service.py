...
SERVICE_NAME = "urban-congestion-index"
DEFAULT_INPUT_TOPICS = [
    "service/mobility/inference/traffic-flow",
    "service/mobility/inference/traffic-speed",
]
DEFAULT_OUTPUT_TOPIC = "service/mobility/inference/urban-congestion-index"
DEFAULT_SERVICE_URN = "urn:ufcity:service:urban-congestion-index"
DEFAULT_PROPERTY_KIND = "urn:ufcity:propertykind:UrbanCongestionIndex"
...
def _extract_road_segment(feature_of_interest: Any) -> float | None:
    if not isinstance(feature_of_interest, str) or not feature_of_interest:
        return None
...
class UrbanCongestionIndexService(Observer):
    def __init__(
        self,
        *,
    ...
    def _parse_flow_input(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "flow_confidence": _safe_float(result.get("urn:ufcity:confidence")),
            "flow_feature_of_interest": message.get("saref:hasFeatureOfInterest"),
            "flow_direction": features.get("SENTIDO"),
            "flow_lane": _safe_int(features.get("FAIXA")),
            ...
            "flow_hour": _safe_int(features.get("hour")),
            "flow_weekday": _safe_int(features.get("weekday")),
        }

    def _parse_speed_input(self, message: dict[str, Any]) -> dict[str, Any]:
        result = _extract_result_block(message)
        feature_of_interest = message.get("saref:hasFeatureOfInterest")
        ts = _extract_timestamp(message)
        dt = pd.to_datetime(ts, errors="coerce")

        return {
            "speed_level": result.get("saref:hasValue"),
            "speed_confidence": _safe_float(result.get("urn:ufcity:confidence")),
            "speed_feature_of_interest": feature_of_interest,
            "speed_road_segment": _safe_float(_extract_road_segment(feature_of_interest)),
            "speed_hour": _safe_int(dt.hour) if not pd.isna(dt) else None,
            "speed_weekday": _safe_int(dt.dayofweek) if not pd.isna(dt) else None,
        }

    def _build_model_input(self) -> pd.DataFrame:
        flow_topic = self.input_topics[0]
        speed_topic = self.input_topics[1]
        flow_features = self.latest_by_topic[flow_topic]["features"]
        speed_features = self.latest_by_topic[speed_topic]["features"]

        row = {**flow_features, **speed_features}
        return pd.DataFrame([row], columns=self.features)

    def _compute_congestion_index(self, proba: np.ndarray, classes: list[str]) -> float:
    ...
    def _build_output_message(
        ...
        *,
        ...
        payload: dict[str, Any] = {
            "inputs_summary": {
                "flow_level": self.latest_by_topic[self.input_topics[0]]["features"].get("flow_level"),
                "speed_level": self.latest_by_topic[self.input_topics[1]]["features"].get("speed_level"),
            }
        }
        if congestion_index is not None:
            payload["congestion_index"] = round(congestion_index, 3)
        ...
        if congestion_index is not None:
            message["saref:hasResult"]["urn:ufcity:congestionIndex"] = round(float(congestion_index), 3)
    ...
    def update(self, topic: str, message: Any) -> None:
        ...
        if topic == self.input_topics[0]:
            parsed_features = self._parse_flow_input(message)
        elif topic == self.input_topics[1]:
            parsed_features = self._parse_speed_input(message)
        else:
            return
        ...
        if hasattr(self.pipeline, "predict_proba"):
            ...
            congestion_index = self._compute_congestion_index(proba=proba, classes=classes)
        ...
        print(
            f"[{SERVICE_NAME}] publicado congestion={prediction} "
            f"index={None if congestion_index is None else round(congestion_index, 3)} "
...
def main() -> None:
    ...
    observer = UrbanCongestionIndexService(
